...
if [ $# = 1 ] && [ $1 = "quick" ] ; then
	 ...
	 nohup geth --rpc --rpcapi "web3, eth, net,miner, personal, admin" --mine --etherbase 0  \
	 --unlock 0 --password res/pass --solc $SOLC &> logs/geth.log &
else
	if [ ! -d ~/.ethereum ] || [ ! -f ~/.ethereum/keystore ]; then
		geth account new
	fi
	nohup geth --rpc --rpcapi "web3, eth, miner, net, personal, admin" --mine --etherbase 0  \
...
echo "To connect interactively, run 'node tools console'."
